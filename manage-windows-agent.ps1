...
& {
  ...
  function Show-Logs {
    ...
    # ============================================================
    #  Windows Security  -  moved out of this script
    # ============================================================
    #
    # Defender exclusions, quarantine inspection and the threat list used to
    # live here. A downloaded script that reads the antivirus threat list and
    # then writes exclusions for itself is indistinguishable from malware that
    # does the same, and Defender said so: the whole installer was classified
    # Trojan:PowerShell/Killav.VDA!MTB and AMSI refused to execute any of it, so
    # nothing could be installed at all.
    #
    # That verdict is fair and this file stops arguing with it. Install now does
    # only what an installer does - resolve the server, download a binary,
    # verify its checksum, write a config, register a task - and no longer
    # touches antivirus configuration on the way past.
    #
    # The same code lives in manage-windows-defender.ps1, published beside this
    # script, for an administrator who fetches it deliberately when Defender has
    # taken the agent. See option [6].
  ...
  while ($true) {
    ...
    Write-Host '   [6] Security   ' -ForegroundColor Blue    -NoNewline; Write-Host '- Defender blocked the agent? how to get the security tool'
    ...
    switch ($choice.Trim()) {
      ...
      '6' {
        Blank
        Hr '=' 'Blue'
        Write-Host '   WINDOWS SECURITY' -ForegroundColor Blue
        Hr '=' 'Blue'
        Hint 'Defender diagnostics and the agent exclusion are a separate tool now.'
        Hint 'This installer no longer changes antivirus configuration - doing so got'
        Hint 'the whole script classified as malware and blocked before it could run.'
        Blank
        Hint 'Run this AS ADMINISTRATOR to diagnose or allow the agent:'
        Blank
        $_db = if ($DIST_BASE) { $DIST_BASE } else { 'http://<dlp-server>:55100/api/v1/agent-dist' }
        Hint ("  irm $_db/manage-windows-defender.ps1 -OutFile " + '$env:TEMP\csdlp-defender.ps1')
        Hint '  powershell -ExecutionPolicy Bypass -File $env:TEMP\csdlp-defender.ps1'
        Blank
        Hint 'It saves to disk rather than piping into iex, so you can read it first.'
        Hint 'Expect Defender to flag it too - registering an exclusion IS the'
        Hint 'behaviour being detected. The durable fix is an Authenticode signature.'
        Blank
