...
- `--service-name` (default `cybersentineldlp-agent`)
- `--log-path` (default `<install-dir>/cybersentineldlp_agent.log`)
- `--no-start` (skip starting service)
- `--force` (re-clone and overwrite config/venv)
- If `CYBERSENTINELDLP_SERVER_URL` is set and you omit `--manager-url`,
...
- The service autostarts on boot (`WantedBy=multi-user.target`),
  `Restart=on-failure`, and gets `CYBERSENTINELDLP_SERVER_URL` from
  `--manager-url`.
- Config is written to `<config-dir>/agent_config.json`. Existing
  config is preserved unless `--force` is used.
- Logs go to `<install-dir>/cybersentineldlp_agent.log` and to journalctl.

### Useful commands

```bash
systemctl status cybersentineldlp-agent
journalctl -u cybersentineldlp-agent -n 50
sudo tail -n 50 /opt/cybersentineldlp/agent/cybersentineldlp_agent.log
sudo systemctl start|stop|restart cybersentineldlp-agent
```
