...
if (-not (Test-Path $INSTALL_DIR)) { Die "Agent not found at $INSTALL_DIR - run install-agent.ps1 first." }
...
if ($expected -ne $actual) {
    ...
    Die "SHA-256 mismatch - download corrupt. Installed agent left untouched.`n    expected $expected`n    actual   $actual"
}
...
Remove-Item $tmpExe -Force -ErrorAction SilentlyContinue
if (-not $replaced) {
    Die "Could not replace $exePath (file still locked). Agent is stopped - retry the command, or reboot and retry."
}
